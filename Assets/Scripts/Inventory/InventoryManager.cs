...
/*
        ...
        for (int p = 0; p < inventorySlots.Length; p++) {
            if (p > 9) { break; } //Only affect the top 10 slots
            GameObject quickInventorySlot = inventorySlots[p];
            if (quickInventorySlot.transform.childCount == 3) { //Ensure that we aren't dealing with an empty slot
                GameObject currentItemPrefab = quickInventorySlot.transform.GetChild(2).gameObject; //Get the 3rd child
                IItem.enableScript<DroppableObject3D>(currentItemPrefab);
            }
        }
*/
    }
